...
class ClientController
{
    ...
    /**
     * 根据状态类型查询 Datatable JSON Object
     * @param $vars
     */
    public function getMyCertificateList($vars){
        ...
        $allCertificates = Capsule::table('tbltrustocean_certificate')
            ->where('uid',$_SESSION['uid'])->whereIn('status', $whereIn)->forPage($pageStart/$pageLength + 1, $pageLength)
            ->orderBy('id','desc')
            ->get();

        $recordsCount = Capsule::table('tbltrustocean_certificate')
            ->where('uid', $_SESSION['uid'])->whereIn('status', $whereIn)->count('id');
    ...
    private function formatOutputAndClean($certificateObject){
        ...
        foreach ($sslArray as $key=>$sslitem){
            ...
            switch ($sslitem['period']){
                case 'Quarterly':
                    $periodItem = '<span class="tag-w1">3个月</span>';
                    break;
                case 'Annually':
                    $periodItem = '<span class="tag-w1">12个月</span>';
                    break;
                case 'Biennially':
                    $periodItem = '<span class="tag-w1">24个月</span>';
                    break;
                default:
                    $periodItem = '<span class="tag-w1">3个月</span>';
